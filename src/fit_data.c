...
fit_data(uint8_t const *data, t_opt *options) {
	...
	new_data[options->new_size - 6] = (uint8_t)((fill_size & 0x0000ff0000000000) >> 40);
	new_data[options->new_size - 5] = (uint8_t)((fill_size & 0x000000ff00000000) >> 32);
	new_data[options->new_size - 4] = (uint8_t)((fill_size & 0x00000000ff000000) >> 24);
